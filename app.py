...
# Pre-load 3 test issues (in-memory DB)
issues_db = [
    {
        "id": str(uuid.uuid4())[:8],
        "title": "Login Bug",
...
def get_issues():
    ...
    if sort_by == 'title':
        ...
        # Fix ISO parsing
        filtered_issues.sort(key=lambda x: datetime.fromisoformat(x['updatedAt'].rstrip('Z') + '+00:00' if x['updatedAt'].endswith('Z') else x['updatedAt']), reverse=reverse)
